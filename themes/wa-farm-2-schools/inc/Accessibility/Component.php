...
class Component implements Component_Interface {
	...
	public function action_enqueue_navigation_script() {
		...
		wp_enqueue_script(
			'wa-farm-2-schools-navigation',
			get_theme_file_uri( '/assets/js/navigation.min.js' ),
			array(),
			wa_farm_2_schools()->get_asset_version( get_theme_file_path( '/assets/js/navigation.min.js' ) ),
			false
		);
		wp_script_add_data( 'wa-farm-2-schools-navigation', 'async', true );
		wp_script_add_data( 'wa-farm-2-schools-navigation', 'precache', true );
		wp_localize_script(
			'wa-farm-2-schools-navigation',
			'waFarm2SchoolsScreenReaderText',
			array(
				'expand'   => __( 'Expand child menu', 'wa-farm-2-schools' ),
				'collapse' => __( 'Collapse child menu', 'wa-farm-2-schools' ),
			)
		);
	}
