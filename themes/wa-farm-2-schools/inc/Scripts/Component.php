...
class Component implements Component_Interface {
	...
	protected function get_js_files(): array {
		...
		$js_files = array(
			'wa-farm-2-schools-global-js'  => array(
				'file'         => 'global.min.js',
				'dependencies' => array( 'jquery', 'wa-farm-2-schools-popover-js' ),
				'in_footer'    => true,
			),
			'wa-farm-2-schools-popover-js' => array(
		 ...
		 */
		$js_files       = apply_filters( 'wa_farm_2_schools_js_files', $js_files );
		$this->js_files = array();
	...
	public function register_global() {
		...
		foreach ( $js_files as $handle => $data ) {
			...
			$version = wa_farm_2_schools()->get_asset_version( $js_dir . $data['file'] );
			$asset   = $js_uri . wa_farm_2_schools()->get_asset_path( $data['file'] );
