 ...
 * * `wa_farm_2_schools()->print_styles()`
 */
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'wa-farm-2-schools-fonts', $google_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}

		// Enqueue Typekit fonts if using.
		$typekit_fonts_url = $this->get_typekit_fonts_url();
		if ( ! empty( $typekit_fonts_url ) ) {
			wp_enqueue_style( 'wa-farm-2-schools-typekit', $typekit_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		if ( ! empty( $fontawesome_kit_url ) ) {
			...
			wp_enqueue_script( 'wa-farm-2-schools-fa', $fontawesome_kit_url, array(), null, array( 'in_footer' => true ) ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			$version = wa_farm_2_schools()->get_asset_version( $css_dir . $data['file'] );
			$asset   = $css_uri . wa_farm_2_schools()->get_asset_path( $data['file'] );
			$deps    = array();
			/*
	...
	public function action_add_editor_styles() {
		...
		$google_fonts_url = $this->get_google_fonts_url();
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'wa-farm-2-schools-google-fonts', $google_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
	...
	public function filter_resource_hints( array $urls, string $relation_type ): array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'wa-farm-2-schools-fonts', 'queue' ) ) {
			$urls[] = array(
		...
		$handles   = array_filter(
			...
			function ( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'wa-farm-2-schools' ), $handle ) ), 'WA Farm 2 Schools - Custom Theme 2.0.0' );
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'wa_farm_2_schools_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		 ...
		 */
		return apply_filters( 'wa_farm_2_schools_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files(): array {
		...
		$css_files = array(
			'wa-farm-2-schools-global' => array(
				'file'   => 'global.min.css',
				'global' => true,
			),
		);
		 ...
		 */
		$css_files = apply_filters( 'wa_farm_2_schools_css_files', $css_files );
	...
	protected function get_google_fonts(): array {
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'wa_farm_2_schools_google_fonts', $google_fonts );
